...
- A subset (`test-rsvp-*.mjs`, `test-wfl-continuous.mjs`, `test-landing-mobile-flow.mjs`, `test-visual-editor-live-frame.mjs`, `test-visual-editor-static-preview.mjs`, `test-visual-editor-section-switch.mjs`) hit `http://localhost:4173` — run `npm run build:pages && npm run serve:dist` first.
  Update (admin v2): the three `test-visual-editor-*` scripts and their `scripts/visual-editor-harness.html` fixture tested the old tab-based `assets/js/admin/visual-editor.js` directly (down to a `#ve-section` dropdown that no longer existed even before this rewrite) and were removed when the admin panel was rewritten — see `docs/rencana-admin-v2.md`. Visual Editor now lives at `assets/js/panel/pages/editor-visual.js` (page key `"editor-visual"`), exercised by `scripts/test-admin-v2-contract.mjs` as a static check; no live-frame Playwright coverage was ported.
